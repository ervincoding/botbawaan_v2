	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `Ervin Fahreza`, //Kasih namalu
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database Ervin```'
...
⚠️ Segala kebijakan dan ketentuan Ervin di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 

Arigatou Gozaimasu! Untuk kalian user ramah dan Beberapa orang yg ikut membantu juga dalam project pembuatan Ervin-
😖🙏

For GitHub : https://github.com/
YouTube index : https://youtube.com/c/
`
sendButLocation(from, `${menu}`, "*Ervin Fahreza*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
 ...
 rows: [
                          {
                           ...
                           {
                              "title": "Source Code⚠️",
                              "description": 'Script / Sc Dari Bot ERVIN',
                              "rowId": ""
...
sendButMessage(from, menu, 'By: Ervin Fahreza', but)
break
case 'nsfww':
...
case '3d':
                    ...
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} ERVIN Chan`)
                    txt1 = args[0]
                    txt2 = args[1]
...
case 'pornhub':
            ...
            reply (mess.wait)
              ...
              buttonsMessage = {footerText:'By: Ervin Fahreza', imageMessage: imageMsg,
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						index.sendMessage(from, `By: Ervin Fahreza`, text)
...
katalog(menu)
sendButLocation(from, `Hai ${pushname} ☺️`, "*_© Ervin Fahreza.ೃ࿐_*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}sewa`,buttonText:{displayText:'SEWA 🎟️'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'stopjadibot':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
try {
...
case 'ss':
             ...
             buttons = [{buttonId: `${prefix}menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await index.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'By: Ervin Fahreza', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT Ervin!`,buttons,headerType:4}
              prep = await index.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              index.relayWAMessage(prep)
              break
                    case 'tiktokdl': 
...
┃💌 : wa.me/6283115824395
┃💌 : ==============
┃💌 : Instagram : _*@ervinfahreza061*_
┗━━⬣`
				sendButLocation(from, `${memek}`, ` Poweren By\nErvin!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'rdp':
case 'vps':
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1}]
              imageMsg = (await index.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'ERVIN FAHREZA', imageMessage: imageMsg,
  ...
  //── 「 Pembatas 」 ──//               
			...
			case 'ping':
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					if (err) return index.sendMessage(from, `root @ervinfahreza:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
