...
sendButMessage(from, menu, 'Powered By Ervin', but)
break
case 'maker':
...
💬 Website : -
`
sendButImage(from, menu, `*_Powered By Pembuat_* ketik .allmenu untuk semua menu`, buffer1, [
{
...
`
  ...
  //── 「 Pembatas 」 ──//               
			...
			case 'runtime':
				...
				sendButLocation(from, `${memek}`, `Powered By\nErvin!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
